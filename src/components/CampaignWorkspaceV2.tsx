...
export default function CampaignWorkspaceV2(){
  ...
  const[displayName,setDisplayName]=useState("New Campaign");
  ...
  useEffect(()=>{
    ...
    place();
    fetch("/api/campaigns",{cache:"no-store"}).then(r=>r.ok?r.json():Promise.reject(new Error())).then(d=>setCampaigns((d.campaigns??[]) as Campaign[])).catch(()=>setStatus("Campaign list could not be loaded."));
    const sync=()=>{syncDates();const n=builderName()?.value.trim();if(!selectedId)setDisplayName(n||"New Campaign")};
    document.addEventListener("input",sync,true);document.addEventListener("change",sync,true);
    setTimeout(()=>{setSelectedId("");setDisplayName("New Campaign");announce("","");syncDates()},250);
    return()=>{cancelled=true;document.removeEventListener("input",sync,true);document.removeEventListener("change",sync,true)};
  },[pathname]);

  function setProductSelection(wanted:Set<string>){
    document.querySelectorAll<HTMLTableRowElement>(".campaign-table tbody tr").forEach(row=>{const box=row.querySelector<HTMLInputElement>('input[type="checkbox"]');const sku=text(row.querySelector("td:nth-child(2) small"));if(box&&box.checked!==wanted.has(sku))box.click()});
  }
  function setAssetSelection(wanted:Set<string>){
    document.querySelectorAll<HTMLElement>("#channels .opportunity-channel-card").forEach(card=>{const box=card.querySelector<HTMLInputElement>('.asset-select input[type="checkbox"]');const asset=text(card.querySelector(".asset-select strong"));if(box&&box.checked!==wanted.has(asset))box.click()});
  }
  function setOpportunitySelection(wanted:Record<string,string[]>){
    document.querySelectorAll<HTMLElement>("#channels .opportunity-channel-card").forEach(card=>{const asset=text(card.querySelector(".asset-select strong")),labels=new Set(wanted[asset]??[]);card.querySelectorAll<HTMLElement>(".opportunity-option").forEach(option=>{const box=option.querySelector<HTMLInputElement>('input[type="checkbox"]');const label=text(option.querySelector("strong")).replace(/Paid$/,"" ).trim();if(box&&box.checked!==labels.has(label))box.click()})});
  }

  function startBlank(){
    setSelectedId("");setDisplayName("New Campaign");announce("","");
    ...
    const m=messagingPanel();m?.querySelectorAll<HTMLInputElement>("input").forEach(i=>setNative(i,""));m?.querySelectorAll<HTMLTextAreaElement>("textarea").forEach(t=>setNative(t,""));
    setStart("");setEnd("");setStatus("Blank campaign ready. Choose a campaign type and begin defining it.");
  }

  function resumeCampaign(id:string){
    if(!id){startBlank();return}
    const c=campaigns.find(x=>x.id===id);if(!c){setStatus("That campaign could not be found.");return}
    setSelectedId(id);setDisplayName(c.name);setStatus(`Loading ${c.name}…`);
    const s=parseState(c),name=builderName(),dates=builderDates(),objective=builderObjective();
    if(name)setNative(name,c.name);if(dates[0])setNative(dates[0],dateOnly(c.startDate));if(dates[1])setNative(dates[1],dateOnly(c.endDate));if(objective)setNative(objective,c.objective??"");
    setStart(dateOnly(c.startDate));setEnd(dateOnly(c.endDate));announce(c.id,c.name);
    const skus=new Set(s.productSkus?.length?s.productSkus:(c.products??[]).map(p=>p.product.sku));setProductSelection(skus);
    const assets=new Set(s.activeAssets??[]);setAssetSelection(assets);
    window.setTimeout(()=>{
      setOpportunitySelection(s.opportunities??{});
      const m=messagingPanel(),mi=m?Array.from(m.querySelectorAll<HTMLInputElement>("input")):[],ma=m?Array.from(m.querySelectorAll<HTMLTextAreaElement>("textarea")):[];
      if(mi[0])setNative(mi[0],s.headline??"");if(mi[1])setNative(mi[1],s.cta??"");if(ma[0])setNative(ma[0],s.coreMessage??"");
      setStatus(`Loaded ${c.name} (${c.status}). Continue editing and save whenever you want.`);
    },120);
  }
  ...
  const content=<section style={{width:"100%",margin:"14px 0 22px",display:"grid",gap:12}}>
    <div style={{padding:"16px 20px",borderRadius:15,background:"#183b24",color:"white",boxShadow:"0 8px 24px rgba(25,55,35,.16)"}}><div style={{fontSize:12,fontWeight:900,letterSpacing:".12em",textTransform:"uppercase",opacity:.78}}>Campaign currently being created / edited</div><div style={{fontSize:29,fontWeight:900,marginTop:4}}>{displayName}</div></div>
