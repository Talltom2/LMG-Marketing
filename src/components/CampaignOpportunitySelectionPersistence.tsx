...
function currentTemplateId(){const selected=document.querySelector<HTMLElement>(".template-card.selected strong");const name=text(selected);return campaignTemplates.find(t=>t.name===name)?.id??"EVERGREEN_SPOTLIGHT"}
...
export default function CampaignOpportunitySelectionPersistence(){
  ...
  useEffect(()=>{
    if(location.pathname!=="/campaigns"||!id)return;
    ...
    const persist=async(value:Record<string,string[]>)=>{
      ...
      write(id,normalized);const signature=JSON.stringify(normalized);if(signature===last.current)return;
      try{const r=await fetch(`/api/campaigns/${encodeURIComponent(id)}/builder-state`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:{opportunities:normalized,opportunityLabels:labelsFor(normalized)}})});if(r.ok)last.current=signature}catch{}
    };
      ...
      try{
        document.querySelectorAll<HTMLElement>("#channels .opportunity-channel-card").forEach(card=>{
          ...
          card.querySelectorAll<HTMLElement>(".opportunity-option").forEach(option=>{
            const label=cleanLabel(text(option.querySelector("strong"))),oid=opportunityId(channel,label),box=option.querySelector<HTMLInputElement>('input[type="checkbox"]');
    ...
    const restore=async()=>{
      let saved=read(id);
      try{
        const r=await fetch(`/api/campaigns/${encodeURIComponent(id)}/builder-state`,{cache:"no-store"});
        if(r.ok){const d=await r.json() as {state?:{opportunities?:Record<string,string[]>;opportunityLabels?:Record<string,string[]>}};if(d.state?.opportunities&&typeof d.state.opportunities==="object")saved=d.state.opportunities;else if(d.state?.opportunityLabels){saved=Object.fromEntries(Object.entries(d.state.opportunityLabels).map(([channel,labels])=>[channel,labels.map(label=>opportunityId(channel,label)).filter(Boolean) as string[]]))}}
      }catch{}
      if(cancelled)return;write(id,saved);last.current=JSON.stringify(Object.fromEntries(Object.entries(saved).map(([channel,ids])=>[channel,Array.from(new Set(ids)).sort()])));apply(saved);
    };
    void restore();

    const scanChannel=(card:HTMLElement,current:Record<string,string[]>)=>{
      ...
      if(!active){delete current[channel];return}
      const options=Array.from(card.querySelectorAll<HTMLElement>(".opportunity-option"));
      if(!options.length)return;
      current[channel]=options.filter(o=>o.querySelector<HTMLInputElement>('input[type="checkbox"]')?.checked).map(o=>opportunityId(channel,cleanLabel(text(o.querySelector("strong"))))).filter(Boolean) as string[];
    };

    const scheduleScan=(card?:HTMLElement|null)=>{
      if(saveTimer.current)window.clearTimeout(saveTimer.current);
      saveTimer.current=window.setTimeout(()=>{
        if(restoring.current)return;
        const current={...read(id)};
        if(card)scanChannel(card,current);else document.querySelectorAll<HTMLElement>("#channels .opportunity-channel-card").forEach(c=>scanChannel(c,current));
    ...
    const onClick=(event:MouseEvent)=>{
      ...
      if(template){const name=text(template.querySelector("strong")),t=campaignTemplates.find(x=>x.name===name);if(t){const next=Object.fromEntries(t.recommendedAssets.map(channel=>[channel,recommendedOpportunityIdsFor(channel,t.id)]));window.setTimeout(()=>void persist(next),80)}return}
      ...
      if(reset&&/AI recommendations/i.test(text(reset))){const asset=text(card.querySelector(".asset-select strong")),channel=assetToChannel[asset];if(channel){const current={...read(id),[channel]:recommendedOpportunityIdsFor(channel,currentTemplateId())};window.setTimeout(()=>void persist(current),80)}return}
      if(target.closest('.opportunity-option input[type="checkbox"]')||target.closest('.asset-select input[type="checkbox"]'))scheduleScan(card);
    };

    const keepVisibleInSync=()=>{const saved=read(id);apply(saved)};
    document.addEventListener("click",onClick,true);
    document.addEventListener("change",keepVisibleInSync,true);
    const interval=window.setInterval(keepVisibleInSync,800);
    return()=>{cancelled=true;if(saveTimer.current)window.clearTimeout(saveTimer.current);window.clearInterval(interval);document.removeEventListener("click",onClick,true);document.removeEventListener("change",keepVisibleInSync,true)};
  },[id]);
