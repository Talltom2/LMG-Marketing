...
function handoff(campaignId:string,snap:Snapshot,summary:string){
 try{
  ...
  localStorage.setItem(LEARNING_DRAFT_KEY,JSON.stringify({name:snap.name,objective:snap.objective,productSkus:snap.state.productSkus??[],channels:snap.state.activeChannels?.length?snap.state.activeChannels:["__NONE__"],startDate:snap.startDate,endDate:snap.endDate,templateId:snap.state.templateId,headline:snap.state.headline,coreMessage:snap.state.coreMessage,cta:snap.state.cta,learningSummary:summary}));
...
export default function CampaignDraftPersistence(){
 ...
 async function refresh(){
  try{
   const r=await fetch("/api/campaigns",{cache:"no-store"});if(!r.ok)throw new Error();const d=await r.json(),list=(d.campaigns??[]) as Campaign[];setCampaigns(list);
   try{
    const activeId=localStorage.getItem(ACTIVE_KEY)||"",active=list.find(c=>c.id===activeId&&!closed.has(c.status.toUpperCase()));
    if(active){
     setSelectedId(active.id);
     const state=hasExactSnapshot(active)?parseState(active):legacyRecovery(active);
     campaignHandoff(active,state,`Current campaign: ${active.name}`);
     if(hasExactSnapshot(active))waitForBuilder(()=>applyExactResume(state));
 ...
 async function writeSnapshot(snap:Snapshot,campaignId:string,automatic:boolean){
  ...
  const r=await fetch("/api/campaigns/draft",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({campaignId:campaignId||undefined,name:snap.name,objective:snap.objective,startDate:snap.startDate,endDate:snap.endDate,state:snap.state})}),d=await r.json();
 ...
 useEffect(()=>{
  ...
  const schedule=(event:Event)=>{
   ...
   const el=event.target as HTMLElement|null;if(!el||el.closest("#campaign-snapshot-workspace")||!el.closest(".campaign-shell"))return;
   if(autoTimer.current)window.clearTimeout(autoTimer.current);setAutoState("pending");
   autoTimer.current=window.setTimeout(async()=>{if(autoBusy.current)return;const snap=snapshot();if(!snap.name||!snap.startDate||!snap.endDate)return;autoBusy.current=true;setAutoState("saving");try{await writeSnapshot(snap,selectedId,true);setAutoState("saved");setStatus(`${snap.name} is being saved automatically as you work.`)}catch(e){setAutoState("error");setStatus(e instanceof Error?`Autosave failed: ${e.message}`:"Autosave failed.")}finally{autoBusy.current=false}},900);
  };
  ...
  return()=>{document.removeEventListener("input",schedule,true);document.removeEventListener("change",schedule,true);document.removeEventListener("click",schedule,true);if(autoTimer.current)window.clearTimeout(autoTimer.current)};
 },[pathname,selectedId]);

 function newCampaign(){setSelectedId("");setAutoState("idle");try{localStorage.removeItem(ACTIVE_KEY);localStorage.removeItem(RESUME_KEY);localStorage.setItem(LEARNING_DRAFT_KEY,JSON.stringify({name:" ",objective:" ",productSkus:["__NONE__"],channels:["__NONE__"],startDate:"",endDate:"",templateId:"EVERGREEN_SPOTLIGHT",headline:" ",coreMessage:" ",cta:" ",learningSummary:"Blank campaign"}));window.location.href="/campaigns"}catch{window.location.href="/campaigns"}}

 function choose(id:string){setSelectedId(id);setAutoState("idle");if(!id){newCampaign();return}const c=campaigns.find(x=>x.id===id);if(!c){setStatus("That campaign could not be found.");return}const exact=hasExactSnapshot(c),state=exact?parseState(c):legacyRecovery(c);try{campaignHandoff(c,state,exact?`Exact saved snapshot: ${c.name}`:`Legacy campaign recovered from the data that was actually persisted: ${c.name}`);localStorage.setItem(RESUME_KEY,JSON.stringify(state));window.location.href="/campaigns"}catch{setStatus("Campaign could not be loaded. Please refresh and try again.")}}

 async function saveCurrent(){const snap=snapshot();if(!snap.name){setStatus("Give the campaign a name before saving it.");return}if(!snap.startDate||!snap.endDate){setStatus("Set both campaign dates before saving.");return}setSaving(true);setStatus("Saving the complete campaign snapshot…");try{const saved=await writeSnapshot(snap,selectedId,false);if(saved){setAutoState("saved");setStatus(`${saved.name} saved. Autosave is now active for every subsequent edit.`)}}catch(e){setStatus(e instanceof Error?e.message:"Unable to save campaign.")}finally{setSaving(false)}}
