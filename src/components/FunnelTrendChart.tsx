...
type MetricAvailability=Record<MetricKey,boolean>;
...
export default function FunnelTrendChart({data,labels,availability}:{data:FunnelPoint[];labels:MetricLabels;availability:MetricAvailability}){
  const metrics=metricKeys.map(key=>({key,label:labels[key],color:metricColors[key],available:availability[key]}));
  const initialMetrics=metricKeys.filter(key=>availability[key]);
  const[selected,setSelected]=useState<MetricKey[]>(initialMetrics.length?initialMetrics:["visitors"]);
  const toggle=(key:MetricKey)=>{
    if(!availability[key])return;
    setSelected(current=>current.includes(key)?(current.length===1?current:current.filter(k=>k!==key)):[...current,key]);
  };
  ...
  const series=metrics.filter(m=>m.available&&selected.includes(m.key)).map(metric=>{
  ...
  return <section style={{marginTop:16,padding:18,border:"1px solid #e0e5dc",borderRadius:16,background:"#fff",boxShadow:"0 5px 18px rgba(35,57,35,.05)"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:18,marginBottom:14}}><div><h2 style={{margin:0,color:"#1e2f21"}}>6-Month Funnel Trend</h2><p style={{margin:"4px 0 0",color:"#6d776c"}}>Each point is a complete 7-day total. Unavailable telemetry is identified instead of being shown as a false zero.</p></div><div style={{textAlign:"right",minWidth:160}}><strong style={{display:"block",fontSize:"1rem",color:"#173c1c"}}>{selected.length} metric{selected.length===1?"":"s"} shown</strong><span style={{fontSize:12,color:"#657064"}}>latest 7-day period</span></div></div>
    <div aria-label="Website funnel trend metrics" style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:12}}>{metrics.map(m=>{const active=m.available&&selected.includes(m.key);return <button type="button" aria-pressed={active} aria-disabled={!m.available} disabled={!m.available} key={m.key} onClick={()=>toggle(m.key)} style={{padding:"8px 11px",borderRadius:9,border:active?`2px solid ${m.color}`:"1px solid #d5ddd1",background:active?"#f7faf5":"#fbfcfa",color:active?m.color:"#7b8579",fontWeight:800,cursor:m.available?"pointer":"not-allowed",opacity:m.available?1:.58}}><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:m.color,marginRight:6}}/>{m.label}{!m.available?" · unavailable":latest&&active?` · ${latest[m.key].toLocaleString()}`:""}</button>})}</div>
