...
function rememberForStep7(label:string,url:string){try{const map=JSON.parse(sessionStorage.getItem(BRIDGE_KEY)??"{}") as VisualMap;if(map[label]!==url){map[label]=url;sessionStorage.setItem(BRIDGE_KEY,JSON.stringify(map))}}catch{}}
function rememberApproval(label:string){const all=readApprovals(),ck=campaignKey(),current=new Set(all[ck]??[]);if(!current.has(label)){current.add(label);all[ck]=[...current];writeApprovals(all)}}
function markFallbackApproved(card:HTMLElement,label:string){
  const version=card.querySelector<HTMLElement>(".creative-version");if(version&&!/APPROVED/i.test(text(version)))version.textContent="APPROVED";
  card.classList.remove("status-source","status-recommended");card.classList.add("status-approved");
  const button=card.querySelector<HTMLButtonElement>("[data-lmg-ai-campaign-select='1']");if(button){button.textContent="✓ Saved to Campaign";button.disabled=true}
  rememberApproval(label);
}
...
function ensureSelectControl(card:HTMLElement,label:string,url:string){
  ...
  const approved=(readApprovals()[campaignKey()]??[]).includes(label)||/APPROVED/i.test(text(card.querySelector(".creative-version")));
  ...
  if(native){
    native.disabled=false;native.removeAttribute("title");
    if(approved&&!/APPROVED/i.test(text(card.querySelector(".creative-version"))))window.setTimeout(()=>{if(!native.disabled)native.click()},0);
    return;
  }

  if(approved)markFallbackApproved(card,label);
  let button=card.querySelector<HTMLButtonElement>("[data-lmg-ai-campaign-select='1']");
  if(!button){
    button=document.createElement("button");button.type="button";button.className="primary-button";button.dataset.lmgAiCampaignSelect="1";
    const actions=card.querySelector<HTMLElement>(".creative-actions")??card;actions.insertBefore(button,actions.firstChild);
    button.addEventListener("click",()=>{markFallbackApproved(card,label);document.dispatchEvent(new Event("lmg-builder-state-changed"))});
  }
  button.textContent=approved?"✓ Saved to Campaign":"Select & Save to Campaign";button.disabled=approved;
}
  ...
  useEffect(()=>{
    ...
    const sync=()=>{
      ...
      frame=requestAnimationFrame(()=>{
        ...
        const saved=read();let changed=false;
        for(const card of Array.from(section.querySelectorAll<HTMLElement>("article.creative-card"))){
          ...
          if(!generated&&saved[label]&&isAiCard(card)){
            ...
            preview.innerHTML="";generated=document.createElement("img");generated.src=saved[label];generated.alt=label;generated.dataset.lmgGenerated="1";generated.style.width="100%";generated.style.height="auto";generated.style.borderRadius="12px";preview.appendChild(generated);
          ...
          const url=generated?.src||saved[label];if(url)ensureSelectControl(card,label,url);
        }
        const signature=JSON.stringify(saved);if(changed){write(saved);if(signature!==lastSignature){lastSignature=signature;document.dispatchEvent(new Event("lmg-builder-state-changed"))}}else lastSignature=signature;
      });
    };
    sync();
    const observer=new MutationObserver(sync);observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:["src"]});
