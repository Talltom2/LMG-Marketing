...
export async function GET(){return NextResponse.json({configured:ga4ConfigStatus()})}
...
export async function POST(request:NextRequest){
  try{
    ...
    return NextResponse.json({ok:true,startDate:start,endDate:end,read:rows.length,saved,mapping:{visitors:"GA4 totalUsers (falls back to sessions)",pageViews:"screenPageViews",addToCarts:"addToCarts",checkoutVisits:"checkouts",ordersCompleted:"transactions"}});
