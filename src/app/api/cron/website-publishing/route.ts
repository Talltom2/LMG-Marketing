...
export async function GET(req:NextRequest){
 ...
 const expiredWebsiteAssets=await db.campaignContentAsset.findMany({where:{channel:"WOOCOMMERCE",publicationStatus:"PUBLISHED",campaign:{endDate:{lte:now}}},include:{campaign:true}});
 for(const asset of expiredWebsiteAssets){const confirmation=(asset.adapterConfirmation??{}) as Record<string,any>;try{if(asset.deliverable==="Homepage feature"&&!confirmation.restoredAt&&confirmation.pageId&&confirmation.previousCoverRaw){const restored=await restoreHomepageHero({pageId:Number(confirmation.pageId),coverIndex:Number(confirmation.coverIndex??0),coverRaw:String(confirmation.previousCoverRaw),capturedAt:String(asset.publishedAt?.toISOString()??now.toISOString())});await db.campaignContentAsset.update({where:{id:asset.id},data:{adapterConfirmation:{...confirmation,restoredAt:restored.restoredAt}}});events.push({id:asset.id,event:"RESTORED",campaign:asset.campaign.name,deliverable:asset.deliverable})}else if(["Dedicated landing page","Campaign collection page"].includes(asset.deliverable)&&!confirmation.unpublishedAt&&confirmation.id){const down=await unpublishCampaignPage(Number(confirmation.id));await db.campaignContentAsset.update({where:{id:asset.id},data:{adapterConfirmation:{...confirmation,unpublishedAt:down.unpublishedAt}}});events.push({id:asset.id,event:"UNPUBLISHED",campaign:asset.campaign.name,deliverable:asset.deliverable})}}catch(e){events.push({id:asset.id,event:"EXPIRY_ERROR",campaign:asset.campaign.name,deliverable:asset.deliverable,error:e instanceof Error?e.message:"Unable to end website placement"})}}
 return NextResponse.json({ok:true,checked:actions.length+approvedAssets.length+expiredWebsiteAssets.length,events,executedAt:new Date().toISOString()});
}
