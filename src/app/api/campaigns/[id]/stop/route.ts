...
export async function POST(_req:NextRequest,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const campaign=await db.campaign.findUnique({where:{id},include:{recommendations:{include:{actions:true}}}});
    if(!campaign)return NextResponse.json({error:"Campaign not found."},{status:404});
    const actions=campaign.recommendations.flatMap(r=>r.actions);
    ...
    return NextResponse.json({ok:true,status:"STOPPED",stoppedAt:stoppedAt.toISOString(),outcomes});
