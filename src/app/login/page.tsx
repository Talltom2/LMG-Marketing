...
export default function Login(){
  ...
  return <main className="login-shell"><section className="login-card"><div className="login-mark">LMG</div><p className="eyebrow">Laughing Moose Gifts</p><h1>LMG Marketing</h1><p>Enter the system password to continue.</p><form onSubmit={submit}><label>Password<input autoFocus type="password" value={pw} onChange={e=>setPw(e.target.value)} autoComplete="current-password"/></label>{error&&<p className="login-error">{error}</p>}<button disabled={busy||!pw}>{busy?'Checking…':'Access LMG Marketing'}</button></form></section></main>;
}
