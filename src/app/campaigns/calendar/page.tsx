...
type ParsedResult={publishedAt?:string;unpublishedAt?:string;restoredAt?:string;error?:string};
...
const parseResult=(value?:string|null):ParsedResult=>{if(!value)return{};try{return JSON.parse(value) as ParsedResult}catch{return value.toLowerCase().includes("error")?{error:value}:{}}};
const isOpportunity=(rec:CampaignRecommendation)=>rec.title.endsWith("campaign plan")||rec.title.startsWith("WooCommerce · ")||rec.title.startsWith("Website Homepage · ");
const opportunityName=(rec:CampaignRecommendation)=>rec.title.startsWith("Website Homepage · ")?rec.title.replace("Website Homepage · ",""):rec.title.startsWith("WooCommerce · ")?rec.title.replace("WooCommerce · ",""):rec.title.replace(/ campaign plan$/i,"");
const opportunityStatus=(rec:CampaignRecommendation):OpportunityStatus=>{
  const actions=rec.actions??[];
  const hero=actions.find(a=>a.actionType==="WORDPRESS_HOMEPAGE_HERO");
  const publish=actions.find(a=>a.actionType==="PUBLISH_EXECUTION");
  const schedule=actions.find(a=>a.actionType==="SCHEDULE_EXECUTION");
  const approval=actions.find(a=>a.actionType==="CREATIVE_APPROVED");
  const results=actions.map(a=>parseResult(a.resultSummary));
  if(results.some(r=>r.error))return"FAILED";
  if(results.some(r=>r.publishedAt&&!r.unpublishedAt&&!r.restoredAt))return"PUBLISHED";
  if(publish?.completed)return"PUBLISHED";
  if(results.some(r=>r.unpublishedAt||r.restoredAt)||rec.status?.toUpperCase()==="EXECUTED")return"COMPLETED";
  if(hero&&!hero.completed)return"SCHEDULED";
  if(schedule?.completed)return"SCHEDULED";
  if(approval||rec.status?.toUpperCase()==="APPROVED")return"APPROVED";
  return"DRAFT";
};
...
export default function CampaignCalendarPage(){
  ...
  const items=useMemo<CalendarItem[]>(()=>campaigns.flatMap(c=>{
    ...
    c.recommendations?.forEach(rec=>{
      const status=isOpportunity(rec)?opportunityStatus(rec):undefined;
      rec.actions?.forEach(a=>{
        const result=parseResult(a.resultSummary);
        if(a.actionType==="CALENDAR"&&looksLikeDate(a.executionTarget))base.push({id:a.id,campaignId:c.id,campaignName:c.name,date:dateOnly(String(a.executionTarget)),kind:"CHANNEL",label:a.description,opportunityStatus:status});
        else if(a.actionType==="SCHEDULE_EXECUTION"&&looksLikeDate(a.executionTarget))base.push({id:a.id,campaignId:c.id,campaignName:c.name,date:dateOnly(String(a.executionTarget)),kind:"CHANNEL",label:a.description,opportunityStatus:status});
        else if(a.actionType==="METRICS_REVIEW"&&looksLikeDate(a.executionTarget))base.push({id:a.id,campaignId:c.id,campaignName:c.name,date:dateOnly(String(a.executionTarget)),kind:"REVIEW",label:a.description,opportunityStatus:status});
        if(result.publishedAt&&looksLikeDate(result.publishedAt))base.push({id:`${a.id}-published`,campaignId:c.id,campaignName:c.name,date:dateOnly(result.publishedAt),kind:"PUBLISH",label:`${opportunityName(rec)} published`,opportunityStatus:"PUBLISHED"});
  ...
  return <main className="campaign-ops-shell">
    <header className="ops-header"><div><p className="eyebrow">Laughing Moose Gifts · LMG Marketing</p><h1>Campaign Calendar & Operations</h1><p>Master view of campaigns, promotional timing, opportunity execution, approvals, launch activity, performance reviews and closeout work.</p></div><div className="ops-header-actions"><Link className="button-outline" href="/campaigns">Build New Campaign</Link><Link href="/">Marketing Intelligence</Link></div></header>
