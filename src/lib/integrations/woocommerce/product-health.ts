...
export async function getWebsiteProductHealth(){
 ...
 const rows=products.map(p=>{const id=bySku.get(String(p.sku??"").trim());const funnel=id?(agg.get(id)??zero()):zero();const telemetry=Boolean(id&&agg.has(id));const diagnosis=healthFor(p,funnel,telemetry);const conversion=funnel.productViews>0?funnel.purchases/funnel.productViews:null;const cartRate=funnel.productViews>0?funnel.addToCarts/funnel.productViews:null;const checkoutRate=funnel.addToCarts>0?funnel.checkoutStarts/funnel.addToCarts:null;return{id:p.id,sku:p.sku,name:p.name,price:p.price?n(p.price):null,stock:p.stock_quantity,stockStatus:p.stock_status,visibility:p.catalog_visibility,permalink:p.permalink??null,health:diagnosis.health,reasons:diagnosis.reasons,telemetryAvailable:telemetry,funnel:{...funnel,conversion,cartRate,checkoutRate}};}).sort((a,b)=>rank[a.health]-rank[b.health]||a.name.localeCompare(b.name));
