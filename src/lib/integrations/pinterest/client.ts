...
export async function pinterestRequest<T>(path:string,params:Record<string,string|number|boolean|undefined>={},init:RequestInit={}):Promise<T>{
 const token=process.env.PINTEREST_ACCESS_TOKEN;
 if(!token)throw new Error("Pinterest API access token is not configured.");
 const url=new URL(`${baseUrl}${path}`);
 for(const[k,v]of Object.entries(params))if(v!==undefined)url.searchParams.set(k,String(v));
 const r=await fetch(url,{...init,headers:{Accept:"application/json",Authorization:`Bearer ${token}`,"Content-Type":"application/json",...(init.headers??{})},cache:"no-store"});
