...
function inventoryRows(r: AnyObj): AnyObj[] {
  const candidates = [r?.elements,r?.inventory,r?.inventories,r?.payload?.elements,r?.payload?.inventory,r?.payload?.inventories,r?.data?.elements,r?.data?.inventory,r?.data?.inventories];
...
async function allInventories() {
  ...
  for (let page = 0; page < 100; page++) {
    const requestPath: string = cursor ? `/v3/inventories?limit=50&nextCursor=${encodeURIComponent(cursor)}` : "/v3/inventories?limit=50";
...
function rowSku(row: AnyObj): string {
  return String(row?.sku ?? row?.SKU ?? row?.itemSku ?? row?.itemSKU ?? row?.itemInformation?.sku ?? row?.item?.sku ?? row?.identifier?.sku ?? "").toUpperCase();
}

function inventoryQty(row: AnyObj): number {
  const direct = num(row?.quantity ?? row?.availableToSellQty ?? row?.availToSellQty ?? row?.inventoryCount ?? row?.inventoryData?.availableUnits ?? row?.inventoryData?.onhandUnits);
  if (direct != null) return direct;
  const nodes = row?.shipNodes ?? row?.nodes ?? row?.inventory ?? row?.inventories;
  if (Array.isArray(nodes)) return nodes.reduce((sum: number, node: AnyObj) => sum + (num(node?.availToSellQty ?? node?.availableToSellQty ?? node?.quantity ?? node?.amount ?? node?.inventoryData?.availableUnits ?? node?.inventoryData?.onhandUnits) ?? 0), 0);
  return 0;
}
...
async function pricing() {
  ...
  for (let pageNumber = 0; pageNumber < 100; pageNumber++) {
    const r = await walmartRequest<any>("/v3/price/getPricingInsights", {method: "POST",body: JSON.stringify({ pageNumber })}).catch(() => null);
    const data = r?.data ?? r;
...
export async function getWalmartActiveProductHealth() {
  ...
  const published = items.filter((i) => { const s = status(i); return s.includes("PUBLISH") && !s.includes("UNPUBLISHED"); });
  const unpublished = items.filter((i) => status(i).includes("UNPUBLISHED"));
  const other = items.length - published.length - unpublished.length;
  const rank: Record<Health, number> = { RED: 0, YELLOW: 1, GREEN: 2 };

  const products = published.map((i) => {
    ...
    if (inventory === 0) { health = "RED"; reasons.push("Out of stock"); }
    else if (inventory != null && inventory <= 3) { health = "YELLOW"; reasons.push(`Low inventory (${inventory})`); }
    else if (inventory == null) { health = "YELLOW"; reasons.push("Inventory unavailable"); }
    if (buyBox != null && buyBox < 10) { health = "RED"; reasons.push("Buy Box <10%"); }
    else if (buyBox != null && buyBox < 50) { if (health !== "RED") health = "YELLOW"; reasons.push("Buy Box <50%"); }
    if (competitive === false) { if (health !== "RED") health = "YELLOW"; reasons.push("Price uncompetitive"); }
    if (traffic === "LOW" || traffic === "VERY_LOW") { if (health !== "RED") health = "YELLOW"; reasons.push(`${traffic.replace("_", " ")} traffic`); }
    if (!o) { if (health !== "RED") health = "YELLOW"; reasons.push("Offer telemetry incomplete"); }
  ...
  return { products, summary: { totalCatalog: items.length, publishedActive: published.length, unpublished: unpublished.length, otherStatus: other, inventoryRows: inventories.length, pricingOffers: offers.length } };
}
