...
export async function getCampaignMetrics(id:string){
 ...
 return{campaign:{id:campaign.id,name:campaign.name,objective:campaign.objective,offer:campaign.offer,budget:n(campaign.budget),channels:Array.isArray(campaign.channels)?campaign.channels:[],startDate:campaign.startDate,endDate:campaign.endDate,status:campaign.status},summary,baseline,channels,products,window:{start,end,baselineStart:baseStart,baselineEnd:baseEnd}};
}
